...
check_pdfs <- function(mybib_meta) {
  ...
  assertthat::assert_that(
    ...
    msg = glue::glue(
      "The following pdf files are missing: {paste(names(pdf_missing), sep = ', ')}"
    )
  )
}
...
site_create_locale <- function(lang = "ja") {
  ...
  if (!fs::dir_exists(locale_lang)) {
    fs::dir_create(locale_lang)
  }
...
url_status <- function(x, time_limit = 60) {
  ...
  if (is.null(res$result) || ((httr::status_code(res$result) %/% 200) != 1)) {
    ...
    if (is.null(res$result)) {
      return("no response")
    } # or whatever you want to return on "hard" errors # nolint
...
check_urls <- function(urls) {
  ...
  assertthat::assert_that(
    length(fail) == 0,
    msg = glue::glue(
      "The following URLs are broken: {paste(fail, collapse = ', ')}"
    )
  )
}

# Make a publication list for publication site
# depends on the pubs file (see bib-prep_)
#' @export
make_pub_list <- function(pubs, item) {
  x <- pubs |>
    dplyr::filter(.data[[item_column]] == item_value)
  if (nrow(x) == 0) {
    return(htmltools::HTML("<p>No publications found for this category.</p>"))
  }
  pub_list <- list()
  for (i in 1:nrow(x)) {
...
# new make_pub
#' @export
make_pub <- function(pub_row, index) {
  paste0(
...
# Make a publication list for publication site
# depends on the pubs file (see bib-prep_) .. different version

# make_pub <- function(pubs, item) {
#   x <- as.list(pubs[which(pubs$item == item), ])
#   pub_list <- list()
#   return(htmltools::HTML(paste(unlist(pub_list), collapse = "")))
# }
